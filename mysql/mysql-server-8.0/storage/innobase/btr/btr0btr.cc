...
#ifdef HYU
      incl_data += rec_offs_size_vDriver(offsets, cursor->index, rec_get_status(rec));
#else
    ...
    mem_heap_t **heap)      /*!< in: temporary memory heap */
{
  ...
  while (rec != end_rec) {
    ...
    *offsets =
        rec_get_offsets(rec, cursor->index, *offsets, ULINT_UNDEFINED, heap);
#ifdef HYU
    total_data -= rec_offs_size_vDriver(*offsets, cursor->index, rec_get_status(rec));
#else
  ...
  if (is_leaf && !cursor->index->is_clustered() &&
    ...
    if (next_block->page.size.is_compressed()) {
      ...
      ibuf_update_free_bits_if_full(
          next_block, max_size, rec_offs_size_vDriver(*offsets, cursor->index, rec_get_status(rec)) + PAGE_DIR_SLOT_SIZE);
#else
      ibuf_update_free_bits_if_full(
