...
/* Type definition for toggling */
#define UPDATE_TOGGLE_NORMAL	0x00UL
#define SKIP_TOGGLE_NORMAL		0x01UL
#define UPDATE_TOGGLE_ROLLBACK	0x02UL
#define SKIP_TOGGLE_ROLLBACK	0x03UL

#define DEFAULT_TYPE			0x04UL

static inline void validate_toggle_flag(rec_t* rec, ulint *offsets) {
	if (rec_get_toggle_flag(rec, page_rec_is_comp(rec))) {
			ut_a(rec_get_toggle_flag(rec, page_rec_is_comp(rec)) == rec_get_toggle_flag(rec - rec_offs_size(offsets), page_rec_is_comp(rec - rec_offs_size(offsets))));
		} else {
			ut_a(rec_get_toggle_flag(rec, page_rec_is_comp(rec)) == rec_get_toggle_flag(rec + rec_offs_size(offsets), page_rec_is_comp(rec + rec_offs_size(offsets))));
		}
}

static inline bool btr_cur_update_in_recovery_or_rollback(
		ulint flags,			/*!< in: undo logging and locking flags */
		trx_t *trx)				/*!< in: transaction */
{
	if (!(flags & (BTR_NO_LOCKING_FLAG | BTR_NO_UNDO_LOG_FLAG | BTR_KEEP_SYS_FLAG))) {
		return false;
	}

	/* trx->is_recovered : undo operation in recovery
	   trx->op_info		 : rollback transaction in shutdown */
	return (trx->is_recovered || !strcmp("rollback", trx->op_info));
}

static inline ulint btr_cur_need_update_toggle(
	ulint flags,								/*!< in: undo logging and locking flags */
	rec_t *rec,									/*!< in: record */
	trx_t* trx,									/*!< in: transaction */
...
{
	bool is_recovered_or_rollback;
	trx_id_t trx_id_old;

	is_recovered_or_rollback = btr_cur_update_in_recovery_or_rollback(flags, trx);
	trx_id_old = rec_get_trx_id(rec, index);

	if (is_recovered_or_rollback) {
		if (trx_id_old == trx_id) {
			return UPDATE_TOGGLE_ROLLBACK;
		} else {
			return SKIP_TOGGLE_ROLLBACK;
		}
	} else {
		if (trx_id_old != trx_id) {
...
static inline void btr_cur_need_copy_in_parse(
	rec_t *rec,						/*!< in: record */
	...
	dict_index_t *index)	/*!< in: index of the record */
{
	...
	rec_t* rec_start_toggle_unset;
	rec_t* rec_start_toggle_set;
	ulint rec_size;

	rec_size = rec_offs_size(offsets);
	
	rec_start_toggle_set = rec_get_start(rec, offsets);
	
	if (rec_get_toggle_flag(rec, page_rec_is_comp(rec))) {
		rec_start_toggle_unset = rec_get_start(rec - rec_size, offsets);
	} else {
		rec_start_toggle_unset = rec_get_start(rec + rec_size, offsets);
	}

	if (mach_read_from_6(rec_start_toggle_unset) != 0 &&
					rec_get_trx_id(rec_start_toggle_unset + rec_offs_extra_size(offsets), index) != trx_id) {
			memcpy(rec_start_toggle_set, rec_start_toggle_unset, rec_size);
	}
}

UNIV_INLINE
void btr_cur_toggle_mark_set_clust_rec_log(
	rec_t *rec,						/*!< in: record */
	dict_index_t *index,	/*!< in: index of the record */
	mtr_t *mtr,						/*!< in: mtr */
	ulint rec_size);			/*!< in: rec_offs_size(offsets) */
...
#ifdef HYU
    /* You MUST copy all physical record 
	   We only use redo log for update field, so we should do it 
	   We don't need to copy if trx_id in redo log is same as trx_id in record */
	if (!strcmp(index->name, "PRIMARY") && index->space != dict_sys_t::s_space_id &&
			rec_get_status(rec) == REC_STATUS_ORDINARY) {
		btr_cur_need_copy_in_parse(rec, offsets, trx_id, index);
		}
#endif /* HYU */
                         ...
                         further pages */
{
  ...
  if (page_zip) {
    ut_ad(!index->table->is_temporary());
#ifdef HYU
    if (!btr_cur_update_alloc_zip(page_zip, btr_cur_get_page_cur(cursor), index, offsets, 
																	rec_offs_size_vDriver(offsets, index, 
																	rec_get_status(cursor->page_cur.rec)), false, mtr)) {
#else
  ...
  toggle_type = DEFAULT_TYPE;
  rec_size = rec_offs_size(offsets);

  if (!strcmp(index->name, "PRIMARY") && index->space != dict_sys_t::s_space_id &&
		  rec_get_status(rec) == REC_STATUS_ORDINARY) {
	  
	  toggle_type = btr_cur_need_update_toggle(
			  flags, rec, thr_get_trx(thr), offsets, index, trx_id);
	  
		switch(toggle_type) {

			case UPDATE_TOGGLE_NORMAL:
			// fall through
			case UPDATE_TOGGLE_ROLLBACK:
				// Set toggle flag and change record pointer up to toggle flag
				if (rec_get_toggle_flag(rec, page_rec_is_comp(rec))) {
					btr_rec_set_toggle_flag(rec - rec_size, page_zip, FALSE);
					btr_rec_set_toggle_flag(rec, page_zip, FALSE);
					rec = rec - rec_size;
				} else {
					btr_rec_set_toggle_flag(rec, page_zip, TRUE);
					btr_rec_set_toggle_flag(rec + rec_size, page_zip, TRUE);
					rec = rec + rec_size;
				}
				break;
		case SKIP_TOGGLE_NORMAL:
			break;
		case SKIP_TOGGLE_ROLLBACK:
			// Nothing to do
			goto func_exit;
		default:
			ut_a(false);
  ...
  switch (toggle_type) {
	  ...
	  case UPDATE_TOGGLE_ROLLBACK:
		  was_delete_marked = 
			  rec_get_deleted_flag(btr_cur_get_rec(cursor), page_is_comp(buf_block_get_frame(block)));
		  break;
	  default:
		  was_delete_marked =
  ...
  if (is_hashed) {
    /* TODO: Can we skip this if none of the fields
  ...
  if (toggle_type != UPDATE_TOGGLE_ROLLBACK) {
	  row_upd_rec_in_place(rec, index, offsets, update, page_zip);
		...
		row_upd_rec_in_place(btr_cur_get_rec(cursor), index, offsets, update, page_zip);
	}
#else
  ...
  switch (toggle_type) {
		case UPDATE_TOGGLE_NORMAL:
		  // fall through
		case UPDATE_TOGGLE_ROLLBACK:
			btr_cur_toggle_mark_set_clust_rec_log(rec, index, mtr, rec_offs_size(offsets));
			break;
		default:
			// Nothing to do.
			break;
  ...
  if (toggle_type != UPDATE_TOGGLE_ROLLBACK) {
	  ...
	  btr_cur_update_in_place_log(flags, btr_cur_get_rec(cursor), index, update,
						trx_id, roll_ptr, mtr);
	}
                         ...
                         further pages */
{
  ...
  *offsets = rec_get_offsets(rec, index, *offsets, ULINT_UNDEFINED, heap);
  ...
  if (!*heap) {
#ifdef HYU
    *heap = mem_heap_create(rec_offs_size_vDriver(*offsets, index, rec_get_status(rec)) +
...
#ifdef HYU
  old_rec_size = rec_offs_size_vDriver(*offsets, index, rec_get_status(rec));
#else
	...
	ulint rec_size)      /*!< in: rec_offs_size(offsets) */
{
  ...
  log_ptr = mlog_open_and_write_index(mtr, rec, index,
                                      page_rec_is_comp(rec)
                                          ? MLOG_COMP_REC_CLUST_TOGGLE_MARK
                                          : MLOG_REC_CLUST_TOGGLE_MARK, /* Two more MLOG_TYPE for toggle flag */
								      1 + 2 + 2); /* 1 : toggle flag, 2 : offset */
                                      //1 + 1 + DATA_ROLL_PTR_LEN + 14 + 2);
  ...
  /* If bit sets, value is 1.
	 else, value is 0.

	 You should follow this rule in redo recovery */
  flag = rec_get_toggle_flag(rec, page_rec_is_comp(rec));
  ...
  /* Write offset for both side */
  mach_write_to_2(log_ptr, page_offset(rec));
  log_ptr += 2;

  if (flag) {
	  mach_write_to_2(log_ptr, page_offset(rec - rec_size));
  } else {
	  mach_write_to_2(log_ptr, page_offset(rec + rec_size));
  }
  log_ptr += 2;
    ...
    dict_index_t *index)      /*!< in: index corresponding to page */
{
  ...
  if (page) {
	...
	if (!strcmp(index->name, "PRIMARY") && index->space != dict_sys_t::s_space_id &&
		...
		btr_cur_need_copy_in_parse(rec, rec_get_offsets(rec, index, offsets_, ULINT_UNDEFINED, &heap),
						trx_id, index);
		if (UNIV_LIKELY_NULL(heap)) {
     	   mem_heap_free(heap);
     	}
...
/** Parses the redo log record for delete marking or unmarking of a clustered
 index record.
 @return end of log record or NULL */
byte *btr_cur_parse_toggle_mark_set_clust_rec(
    byte *ptr,                /*!< in: buffer */
    byte *end_ptr,            /*!< in: buffer end */
    page_t *page,             /*!< in/out: page or NULL */
    page_zip_des_t *page_zip, /*!< in/out: compressed page, or NULL */
    dict_index_t *index)      /*!< in: index corresponding to page */
{
  ...
  /* If val is 1, toggle bit should set,
	 else, toggle bit should unset.

	 This should follow the rule in writing redo log */
  val = mach_read_from_1(ptr);
  ptr++;
    ...
    mtr_t *mtr)            /*!< in/out: mini-transaction */
{
  ...
  if (!strcmp(index->name, "PRIMARY") && index->space != dict_sys_t::s_space_id &&
	  ...
	  toggle_type = btr_cur_need_update_toggle(
			  flags, rec, thr_get_trx(thr), offsets, index, thr_get_trx(thr)->id);
...
#ifdef HYU
  /* Write redo log for toggle flag if needed */
  /* The only case need_to_toggle will be true is user-table record which uses OVIRAPTOR */
  if (toggle_type == UPDATE_TOGGLE_NORMAL) {
  	btr_cur_toggle_mark_set_clust_rec_log(rec, index, mtr, rec_offs_size(offsets));
  }
#endif /* HYU */
                 ...
                 before latching any further pages */
{
  ...
  no_compress_needed =
      ...
      btr_cur_can_delete_without_compress(cursor,
										rec_offs_size_vDriver(offsets, cursor->index, rec_get_status(rec)), mtr);
#else
  no_compress_needed =
      !rec_offs_any_extern(offsets) &&
