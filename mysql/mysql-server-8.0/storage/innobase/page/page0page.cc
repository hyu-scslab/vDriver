                              ...
                              if allocation succeeds */
{
  ...
  avl_space = page_get_max_insert_size(page, 1);
	...
	//ib::warn() << "[HYU] avl_space: " << avl_space << ", need: " << need;
  if (avl_space >= need) {
    ...
    mtr_t *mtr)             /*!< in: mtr */
{
  ...
  while (!page_cur_is_after_last(&cur1)) {
    ...
    ins_rec = page_cur_insert_rec_low(cur2, index, cur1_rec, offsets, mtr);
		//ib::warn() << "[HYU] ins_rec: " << ins_rec;
    if (UNIV_UNLIKELY(!ins_rec)) {
			//ib::warn() << "[HYU] error in here?";
...
#ifdef HYU
      s = rec_offs_size_vDriver(offsets, index, rec_get_status(rec2));
#else
                         ...
                         the page record type definition */
{
  ...
  for (;;) {
    ...
    if (page_rec_is_user_rec(rec)) {
      data_size += rec_offs_size_vDriver(offsets, index, rec_get_status(rec));
#else
    if (page_rec_is_user_rec(rec)) {
...
#ifdef HYU
    i = rec_offs_size_vDriver(offsets, index, rec_get_status(rec));
#else
    ...
    const ulint *offsets)      /*!< in: offsets for record */
{
  ...
  if (!rec_offs_any_extern(offsets) &&
      ((page_get_data_size(page) - rec_offs_size_vDriver(offsets, index, rec_get_status(pcur->rec)) <
