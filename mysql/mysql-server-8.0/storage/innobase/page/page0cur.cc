...
#ifdef HYU
    ut_ad(rec_size == rec_offs_size_vDriver(ins_offs, index, rec_get_status(insert_rec)));
    cur_rec_size = rec_offs_size_vDriver(cur_offs, index, rec_get_status(cursor_rec));
#else
...
#ifdef HYU
    mismatch_index = rec_offs_size_vDriver(offsets, index, rec_get_status(cursor_rec)) - (end_seg_len >> 1);
#else
    ...
    mtr_t *mtr)          /*!< in: mini-transaction handle, or NULL */
{
  ...
  ut_ad(!page_rec_is_supremum(current_rec));
	...
	/* HYU_vDriver
	 * Filtering user record to allocate 2x size for OVIRAPTOR.
	 * We expand record size user record only, so we make condition
	 * 'only user record index' && 'only user table' && 'except btree node pointer' */
#ifdef HYU
	/* 1. Get the size of the physical record in the page */
	rec_size = rec_offs_size_vDriver(offsets, index, rec_get_status(rec));
#else
...
#ifdef HYU
		if (rec_offs_size_vDriver(foffsets, index, rec_get_status(free_rec)) < rec_size) {
#else
...
#ifdef HYU
  if (!strcmp(index->name, "PRIMARY") && index->space != dict_sys_t::s_space_id && rec_get_status(rec) == REC_STATUS_ORDINARY) {
    memset(insert_buf + rec_size / 2, 0, rec_size / 2);
  }
 
  /* 3. Create the record */
  if (!strcmp(index->name, "PRIMARY") && index->space != dict_sys_t::s_space_id && 
          rec_get_status(rec) == REC_STATUS_ORDINARY &&
          mach_read_from_8(rec + rec_offs_data_size(offsets))) {
      insert_rec = rec_copy_vDriver(insert_buf, rec, offsets);
  } else {
  ...
  UNIV_MEM_ASSERT_RW(rec_get_start(insert_rec, offsets),
                     rec_offs_size_vDriver(offsets, index, rec_get_status(insert_rec)));
#else
  UNIV_MEM_ASSERT_RW(rec_get_start(insert_rec, offsets),
...
rec_t *page_cur_direct_insert_rec_low(rec_t *current_rec, dict_index_t *index,
  ...
  /* 1. Get the size of the physical record in the page */
	// [HYU] TODO: uncompressed record insertion case
#ifdef HYU
  rec_size = index->rec_cache.rec_size * 2;
#else
...
#ifdef HYU
	if (!strcmp(index->name, "PRIMARY") && index->space != dict_sys_t::s_space_id &&
						rec_get_status(insert_rec) == REC_STATUS_ORDINARY)
		memset(insert_buf + rec_size / 2, 0, rec_size / 2);
#endif
...
#ifdef HYU
      if (rec_offs_size_vDriver(foffsets, index, rec_get_status(free_rec)) < rec_size - extra_size_diff) {
#else
    ...
    mtr_t *mtr)          /*!< in: mtr */
{
  ...
  do {
    ...
    if (!strcmp(index->name, "PRIMARY") && index->space != dict_sys_t::s_space_id && 
            rec_get_status(rec) == REC_STATUS_ORDINARY &&
            mach_read_from_8(rec + rec_offs_data_size(offsets))) {
        insert_rec = rec_copy_vDriver(heap_top, rec, offsets);
    } else {
...
#ifdef HYU
  if (rec_get_toggle_flag(prev_rec, page_is_comp(page))) {
      ut_a(rec_get_toggle_flag(prev_rec, page_is_comp(page)) == rec_get_toggle_flag(prev_rec + 206, page_is_comp(page)));
  }
   
  if (rec_get_toggle_flag(current_rec, page_is_comp(page))) {
      ut_a(rec_get_toggle_flag(rec, page_is_comp(page)) == rec_get_toggle_flag(rec + 206, page_is_comp(page)));
  }
 
  if (rec_get_toggle_flag(next_rec, page_is_comp(page))) {
      ut_a(rec_get_toggle_flag(next_rec, page_is_comp(page)) == rec_get_toggle_flag(next_rec + 206, page_is_comp(page)));
  }
#endif /* HYU */ // JAESEON
