    ...
    ulint mode)     /*!< in: BTR_MODIFY_LEAF or BTR_MODIFY_TREE */
{
  ...
  /* If toggle flag is set,
     ...
     Modify undo operation ( Update / Delete ) will find record based on toggle flag */
  bool toggle_flag;
  rec_t* rec_ = btr_cur_get_rec(btr_cur);
  if ((toggle_flag = rec_get_toggle_flag(rec_, page_rec_is_comp(rec_)))) {
      /* Build temporary offsets */
      ...
      temp_offsets = rec_get_offsets(rec_, btr_cur_get_index(btr_cur),
              temp_offsets, ULINT_UNDEFINED, &temp_heap);
      
      btr_cur->page_cur.rec = rec_ + rec_offs_size(temp_offsets);
