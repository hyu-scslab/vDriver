...
static void row_merge_write_rec_low(
    ...
    const ulint *offsets, /*!< in: offsets of mrec */
		const dict_index_t *index) /*!< in: [HYU] dictionary index pointer for vDriver */
#ifndef UNIV_DEBUG
#define row_merge_write_rec_low(b, e, size, fd, foffs, mrec, offsets, index) \
  row_merge_write_rec_low(b, e, mrec, offsets, index)
#endif /* !UNIV_DEBUG */
{
...
#ifdef HYU
  memcpy(b, mrec - rec_offs_extra_size(offsets),
					rec_offs_size_vDriver(offsets, index, rec_get_status(mrec)));
  DBUG_ASSERT(b + rec_offs_size_vDriver(offsets, index, rec_get_status(mrec)) == end);
#else
  memcpy(b, mrec - rec_offs_extra_size(offsets), rec_offs_size(offsets));
  DBUG_ASSERT(b + rec_offs_size(offsets, index) == end);
#endif
  DBUG_VOID_RETURN;
}
		...
		const dict_index_t *index) /*!< in: [HYU] dictionary index pointer for vDriver */
{
  ...
  if (UNIV_UNLIKELY(b + size >= &block[srv_sort_buf_size])) {
    ...
    row_merge_write_rec_low(b, extra_size, size, fd, *foffs, mrec, offsets, index);
    b += size;
  }
...
#define ROW_MERGE_WRITE_GET_NEXT_LOW(N, INDEX, AT_END)                       \
  do {                                                                       \
    b2 = row_merge_write_rec(&block[2 * srv_sort_buf_size], &buf[2], b2,     \
                      of->fd, &of->offset, mrec##N, offsets##N, INDEX);      \
