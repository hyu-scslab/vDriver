                               ...
                               latching any further pages */
{
  ...
  /* If toggle flag is set,
     Change page cursor's record pointer temporarily.
     Operation will find record based on toggle flag */
  bool toggle_flag;
  const rec_t *rec_;
  rec_ = btr_cur_get_rec(cursor);
  
  if ((toggle_flag = rec_get_toggle_flag(rec, dict_table_is_comp(cursor->index->table)))) {
            mem_heap_t *temp_heap = NULL;
      ulint temp_offsets_[REC_OFFS_NORMAL_SIZE];
      ulint *temp_offsets = temp_offsets_;
      rec_offs_init(temp_offsets_);
      temp_offsets = rec_get_offsets(rec_, btr_cur_get_index(cursor),
              temp_offsets, ULINT_UNDEFINED, &temp_heap);
      cursor->page_cur.rec = const_cast<rec_t*>(rec) + rec_offs_size(temp_offsets);
      rec = const_cast<rec_t*>(rec) + rec_offs_size(temp_offsets);
      if (UNIV_LIKELY_NULL(temp_heap)) {
...
#ifdef HYU
  if (toggle_flag) {
      /* restore page cursor's record pointer */
      cursor->page_cur.rec = const_cast<rec_t*>(rec_);
  }
#endif /* HYU */
    return (err);
  }
...
#ifdef HYU
  if (rec_get_toggle_flag(rec, rec_offs_comp(offsets))) {
    return (!rec_get_deleted_flag(rec + rec_offs_size(offsets), rec_offs_comp(offsets)));
  }
#endif /* HYU */
...
static dberr_t row_ins_index_entry(dict_index_t *index, dtuple_t *entry,
  ...
  if (index->is_clustered()) {
		//ib::warn() << "[HYU] row_ins_index_entry";
