...
#ifdef HYU
  ut_ad(rec_offs_size_vDriver(offsets, index, rec_get_status(rec)) <= sizeof index->online_log->tail.buf);
#else
                              ...
                              and a PRIMARY KEY is being created) */
{
  ...
  mrec_size = ROW_LOG_HEADER_SIZE + (extra_size >= 0x80) +
              rec_offs_size_vDriver(offsets, index, rec_get_status(rec)) - omit_size;
#else
  mrec_size = ROW_LOG_HEADER_SIZE + (extra_size >= 0x80) +
