...
#ifdef HYU
		prebuilt->old_vers_heap = mem_heap_create(rec_offs_size_vDriver(*offsets, clust_index,
																							rec_get_status(rec)));
#else
  ...
  for (/* No op */; err == DB_SUCCESS;
    ...
    if (prebuilt->template_type == ROW_MYSQL_DUMMY_TEMPLATE) {
      ...
      memcpy(buf + 4, ret_rec - rec_offs_extra_size(offsets),
             rec_offs_size_vDriver(offsets, index, rec_get_status(ret_rec)));
#else
      memcpy(buf + 4, ret_rec - rec_offs_extra_size(offsets),
  ...
  if (prebuilt->select_lock_type != LOCK_NONE) {
    ...
    switch (err) {
      ...
      case DB_SUCCESS:
		...
		/* Change record pointer if needed.
		   The case rec_get_toggle_flag() != 0 is USER-TABLE ONLY 
		   This change will help build "update vector" */
		if (rec_get_toggle_flag(rec, comp)) {
			rec = rec + rec_offs_size(offsets);
		}
#endif /* HYU */
        break;
      case DB_SKIP_LOCKED:
...
#ifdef HYU
      /* If toggle flag is set, 
         We first look at the latest version in index */
      if (rec_get_toggle_flag(rec, comp)) {
        rec = rec + rec_offs_size(offsets); 
      }
...
#endif /* HYU */
#ifdef HYU_CHAIN
			/* This is used to find the length of version chain */
			trx->read_view->loop_cnt = 0;
#endif /* HYU_CHAIN */
    ...
    if (rec_get_toggle_flag(rec, page_rec_is_comp(rec))) {
			version = rec - rec_offs_size(offsets);
		} else {
			version = rec + rec_offs_size(offsets);
		}
		
		trx_id_t trx_id = rec_get_trx_id(version, index);
		rec_t *prev_version;
  	rec_t *old_vers_vd;

		if (!trx->read_view->changes_visible(trx_id, index->table->name)) {
  			ulint pk_len;
				byte* vd_buf;
  			const byte* pk = rec_get_nth_field_instant(rec, offsets, 
		  		dict_col_get_no(index->fields->col), index, &pk_len);
				...
				vd_buf = 
						static_cast<byte*>(mem_heap_alloc(prebuilt->old_vers_heap, rec_offs_size(offsets)));
				
				(void)vDriver_read_ver(trx->read_view, pk, pk_len, &prev_version, /*&vd_buf,*/ 
								thd_thread_id(trx->mysql_thd), index);
			old_vers_vd = rec_copy(vd_buf, prev_version + rec_offs_extra_size(offsets), offsets);
			ut_free(prev_version);
			//old_vers_vd = prev_version;
			//old_vers_vd = vd_buf;
		} else {
			old_vers_vd = const_cast<byte*>(version);
		}

		err = DB_SUCCESS;
		old_vers = old_vers_vd;
		  ...
		  if (trx->read_only && pk[3] == 1)
					ib::warn() << "[HYU_CHAIN]: " << trx->read_view->loop_cnt;
#endif /* HYU_CHAIN */
  ...
  if (record_buffer != nullptr ||
    ...
    if (UNIV_UNLIKELY(prebuilt->template_type == ROW_MYSQL_DUMMY_TEMPLATE)) {
      ...
      memcpy(buf + 4, result_rec - rec_offs_extra_size(offsets),
             rec_offs_size_vDriver(offsets, index, rec_get_status(result_rec)));
#else
      memcpy(buf + 4, result_rec - rec_offs_extra_size(offsets),
             rec_offs_size(offsets));
#endif
...
#ifdef HYU
			ulint rec_size = rec_offs_size_vDriver(offsets, index, rec_get_status(result_rec));
#else
