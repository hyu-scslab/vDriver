...
#ifdef HYU
  if (toggle_flag != rec_get_toggle_flag(rec, page_rec_is_comp(rec))) {
	  btr_rec_set_toggle_flag(rec, page_zip, toggle_flag);
  }
#endif /* HYU */
                       ...
                       que_thr_t *thr)   /*!< in: query thread */
{
  ...
  /* If toggle flag is set, 
	 ...
	 Modify operation ( Update / Delete ) will find record based on toggle flag. */
  bool toggle_flag;
  if ((toggle_flag = rec_get_toggle_flag(rec, page_rec_is_comp(rec)))) {
	  pcur->m_btr_cur.page_cur.rec = rec + rec_offs_size(offsets);
  }
#endif /* HYU */
  if (node->is_delete) {
...
exit_func:
  ...
  if (toggle_flag) {
	  pcur->m_btr_cur.page_cur.rec = rec;
  }
#endif /* HYU */
  return (err);
}
