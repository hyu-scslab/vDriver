    ...
    mtr_t *mtr)       /*!< in: mtr */
{
  ...
  if (type == TRX_UNDO_INSERT) {
    ...
    ut_a(mach_read_from_2(undo_page + TRX_UNDO_PAGE_HDR + TRX_UNDO_PAGE_TYPE) ==
         TRX_UNDO_UPDATE);
#ifdef HYU
    /* Procedure of undo page initialization in trx_undo_seg_create() */
	trx_usegf_t *seg_hdr;
	trx_upagef_t *page_hdr;
	...
	trx_undo_page_init(undo_page, undo->type, mtr);
...
#ifdef HYU
    /**
			Because of using version driver,
			Treat update undo like insert undo.
			*/
		state = TRX_UNDO_TO_FREE;
#else
...
@param[in]	noredo		whether the undo tablespace is redo logged */
void trx_undo_update_cleanup_vdriver(trx_undo_ptr_t *undo_ptr, bool noredo) {
  trx_undo_t *undo;
  trx_rseg_t *rseg;
