...
static void vdriver_report_modify(trx_t*, dict_index_t*, const rec_t*, const ulint*, const upd_t*, ulint, const dtuple_t*);
#endif
		...
		mtr_t *mtr)           /*!< in: mtr */
{
	...
	*ptr++ = (byte)rec_get_info_bits(rec, dict_table_is_comp(table));
	...
	/* Write to the vDriver version about this change.
	   Update or delete query MUST call vdriver_report_modify. */
#ifdef HYU
	(void)vdriver_report_modify(trx, index, rec, offsets, update, cmpl_info, row);
#endif
	...
	if (row_upd_changes_field_size_or_external(index, offsets, update)) {
		...
		*old_vers = rec_convert_dtuple_to_rec(buf, index, entry, n_ext);
		...
		buf = static_cast<byte *>(mem_heap_alloc(heap,
					rec_offs_size_vDriver(offsets, index, rec_get_status(rec))));
#else
...
#ifdef HYU
// [HYU] vDriver
// Run OVIRAPTOR protocol about record update/delete case.
// We only support in-place update.
void vdriver_report_modify(
		/*========================*/
		trx_t *trx,           /*!< in: transaction */
		dict_index_t *index,  /*!< in: clustered index where update or
								delete marking is done */
		...
		ulint cmpl_info,      /*!< in: compiler info on secondary
								index updates */
		const dtuple_t *row)  /*!< in: clustered index row contains virtual column info */
{
	ut_a(index->is_clustered());
	ut_ad(rec_offs_validate(rec, index, offsets));

	/* In this case, we deal with update & delete cases both */
	/* We only care about modification in user-table */
	if (!strcmp(index->name, "PRIMARY") && index->space != dict_sys_t::s_space_id 
			&& rec_get_status(rec) == REC_STATUS_ORDINARY) {

		byte* rec_start_toggle_unset;
		byte* rec_start_toggle_set;
		ulint rec_size;
		ulint toggle_flag;

		/* If opeartion is done by same transaction,
		   we MUST return immediately */
		if (rec_get_trx_id(rec, index) == trx->id) {
			return;
		}
		...
		/* rec_start_toggle_unset means starting point of record whose toggle flag is unset.
		   vice versa */
		rec_start_toggle_set = rec_get_start(const_cast<rec_t*>(rec), offsets);	
		if (toggle_flag) {
			rec_start_toggle_unset = rec_get_start(const_cast<rec_t*>(rec) - rec_size, offsets);
		} else {
			rec_start_toggle_unset = rec_get_start(const_cast<rec_t*>(rec) + rec_size, offsets);
		}

		/* Don't make new version in vdriver in the first update.
		   TRX_ID_MAX means that the transcation id is changed to TRX_ID_MAX in rollback, so it is uncommitted version */
		if (mach_read_from_6(rec_start_toggle_unset) == 0 || rec_get_trx_id(rec_start_toggle_unset + rec_offs_extra_size(offsets), index) == rec_get_trx_id(rec, index)) {
			memcpy(rec_start_toggle_unset, rec_start_toggle_set, rec_size);
		} else {
			vVersion *version = (vVersion*)ut_malloc_nokey(sizeof(vVersion));
			ulint key_len;
			const byte* primary_key = rec_get_nth_field_instant(rec, offsets, 
					dict_col_get_no(index->fields->col), index, &key_len);

			vnum_t v_start = rec_get_trx_id(rec_start_toggle_unset + rec_offs_extra_size(offsets), index);
			vnum_t v_end = rec_get_trx_id(rec_start_toggle_set + rec_offs_extra_size(offsets), index);

			/* Copy record which will be evicted in vDriver */
			memcpy(version->version, rec_start_toggle_unset, rec_size);
			
			version->length = rec_offs_size(offsets);
			version->primary_key = primary_key;
			version->pk_len = key_len;
			
			(void)vDriver_insert_ver(v_start, v_end, version, index->space);
			
			ut_free(version);
			/* Copy the latest record to adjacent record */
			memcpy(rec_start_toggle_unset, rec_start_toggle_set, rec_size);
		}

		ut_a(memcmp(rec_start_toggle_unset, rec_start_toggle_set, rec_size) == 0);
