...
#else 
      (void)trx_undo_set_state_at_finish(trx->rsegs.m_redo.update_undo, mtr);
#endif /* HYU */
...
#ifndef HYU
      /* We do cleanup in trx_commit_in_memory.
         We follow the procedure of insert_undo in Mysql */
...
#ifdef HYU
    /* We did deal with m_redo case only */
    if (trx->rsegs.m_redo.update_undo != NULL) {
        trx_undo_update_cleanup_vdriver(&trx->rsegs.m_redo, false);
    }
...
#ifdef HYU
	ib_uint64_t lifetime = trx_sys->max_trx_id - trx->id;
	double temp = (double)delta_LLT * (double)TRX_W_AVG
									+ (double)lifetime * (double)TRX_W_NEW;
	delta_LLT = (ib_uint64_t)temp;
#endif
