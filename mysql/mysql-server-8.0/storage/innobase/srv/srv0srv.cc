...
#ifdef HYU_PURGE
/** Get IndexPurge class from row0import.cc */
...
class IndexPurge {
 public:
  ...
  IndexPurge(trx_t *trx, dict_index_t *index, my_thread_id thd_id) UNIV_NOTHROW : m_trx(trx),
                                                             m_index(index),
                                                             m_n_rows(0),
                                                             m_thr_id(thd_id) {
 ...
 private:
  ...
  my_thread_id m_thr_id; /*!< Thread id */
};
...
dberr_t IndexPurge::garbage_collect() UNIV_NOTHROW {
  ...
  while ((err = next()) == DB_SUCCESS) {
    ...
    offsets = offsets_;
    offsets = rec_get_offsets(rec, btr_cur_get_index(btr_pcur_get_btr_cur(&m_pcur)),
                offsets, ULINT_UNDEFINED, &heap);
    pk = rec_get_nth_field_instant(rec, offsets,
            dict_col_get_no(m_index->fields->col), m_index, &pk_len);
 
    ibool deleted;

		if (rec_get_toggle_flag(rec, comp) != rec_get_toggle_flag(rec + rec_offs_size(offsets), comp)) {
			rec_print(stderr, rec, m_index);
			rec_print(stderr, rec + rec_offs_size(offsets), m_index);
			ut_a(false);
		}

    if (rec_get_toggle_flag(rec, comp)) {
      deleted = rec_get_deleted_flag(rec + rec_offs_size(offsets), comp);
      trx_id_n = rec_get_trx_id(rec + rec_offs_size(offsets), m_index);
      trx_id_o = rec_get_trx_id(rec, m_index);
    } else {
      deleted = rec_get_deleted_flag(rec, comp);
      trx_id_n = rec_get_trx_id(rec, m_index);
			if (deleted) {
      	trx_id_o = rec_get_trx_id(rec + rec_offs_size(offsets), m_index);
			}
    }
    ...
    if (!deleted) {
      ...
      if (vDriver_check_removability(pk, pk_len, trx_id_n, trx_id_o, m_thr_id, m_index)) { 
				ut_a(!page_rec_is_supremum(rec) && !page_rec_is_infimum(rec));
				ib::warn() << "Purge rec : " << static_cast<void*>(rec);	
        purge();
      }
    }
  }
  ...
  /* Free heap memory allocated above. */
...
#ifdef HYU
/** Event to signal the vdriver dead thread */
os_event_t srv_vdriver_dead_event;

/** Event to signal the vdriver cut thread */
os_event_t srv_vdriver_cut_event;

/** Event to signal the vdriver stats thread */
os_event_t srv_vdriver_stats_event;
#endif
...
#ifdef HYU
  srv_vdriver_dead_event = os_event_create(0);

  srv_vdriver_cut_event = os_event_create(0);

  srv_vdriver_stats_event = os_event_create(0);
#endif
...
#ifdef HYU
  os_event_destroy(srv_vdriver_dead_event);

  os_event_destroy(srv_vdriver_cut_event);

  os_event_destroy(srv_vdriver_stats_event);
#endif /* HYU */
...
void srv_worker_thread() {
  ...
  do {
    ...
    os_event_wait(slot->event);
#if !defined(HYU) && !defined(HYU_PURGE)
    if (srv_task_execute()) {
...
#ifdef HYU_PURGE
static ibool srv_do_purge_vDriver(que_thr_t* thr, THD* thd) 
{ 
  dberr_t err; 
  dict_table_t *table;
  dict_index_t *index;
  int cmpret1, cmpret2, cmpret3;
  ...
  /* Get table from dict_sys */
  mutex_enter(&dict_sys->mutex);
 
  for (table = UT_LIST_GET_FIRST(dict_sys->tables); table; 
      table = UT_LIST_GET_NEXT(tables, table)) {
    /* skip system table and SDI table */
    cmpret1 = strncmp(table->name.m_name, "mysql/", 6);
    cmpret2 = strncmp(table->name.m_name, "SDI_", 4);
    cmpret3 = strncmp(table->name.m_name, "sys/", 4);

    if (cmpret1 != 0 && cmpret2 != 0 && cmpret3 != 0) { 
      break;
    } 
  }
  ...
  if (err != DB_SUCCESS) {
    lock_cancel_waiting_and_release_vDriver(trx->lock.wait_lock);
    trx_commit_for_mysql(trx);
    trx_free_for_mysql(trx);
    return false;
  }
  ...
  do {
    /* This is physical delete sequence for user record, so get primary index */
    ...
    if (index == NULL) {
      ...
      goto find_next_table;
    }

    ut_a(index != NULL);

    IndexPurge *purge;
    purge = new IndexPurge(trx, index, thd_thread_id(thd));
    ...
    fil_space_t *space;
    space = fil_space_acquire(table->space);
...
find_next_table:
    ...
    for (table = UT_LIST_GET_NEXT(tables, table); table;
        table = UT_LIST_GET_NEXT(tables, table)) {
      /* skip system table */
      cmpret1 = strncmp(table->name.m_name, "mysql/", 6);
      cmpret2 = strncmp(table->name.m_name, "SDI_", 4);
    ...
    /* We scan all table, one purge sequence is done */
    if (table == NULL) {
      mutex_exit(&dict_sys->mutex);
      trx_commit_for_mysql(trx);
      trx_free_for_mysql(trx);
    ...
    if (srv_purge_should_exit(n_pages_purged)) {
      trx_commit_for_mysql(trx);
      trx_free_for_mysql(trx);
      break;
    }

  } while (!srv_purge_should_exit(n_pages_purged) && purge_sys->state == PURGE_STATE_RUN);
...
void srv_purge_coordinator_thread() {
  ...
  do {
    ...
    ibool purge_done = false;
    rw_lock_x_lock(&purge_sys->latch);
    purge_sys->running = false;
    rw_lock_x_unlock(&purge_sys->latch);
    /* wait 10 secs */
    os_thread_sleep(1000000LL);
    ...
    n_total_purged = 0; 
    purge_done = srv_do_purge_vDriver(thr, thd);
#endif /* HYU_PURGE */
